...
export default async function PainelAdminPage() {
  ...
  const [{ data: profile }, { data: agendamentos }, { data: barbeiros }] = await Promise.all([
    ...
    supabase
      .from("agendamentos")
      .select(
        "*, clientes(profile_id, qtd_no_show, bloqueado, profiles(nome)), barbeiros(profile_id, is_dono, profiles(nome)), servicos(nome, preco, duracao_minutos), comandas(id, valor_produtos, comanda_itens(quantidade, preco_unitario, produtos(nome)))"
      )
